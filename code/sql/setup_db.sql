...
BEGIN
	UPDATE TIME_SHEET
	SET USED_TIME = NEW.TO_DATE - NEW.FROM_DATE - NEW.BREAK - 900
	WHERE PERSON_ID = NEW.PERSON_ID AND WORK_DATE = NEW.WORK_DATE;
END;

CREATE TRIGGER CALC_USED2_TIME_SHEET AFTER UPDATE ON TIME_SHEET
WHEN
	OLD.FROM_DATE <> NEW.FROM_DATE OR OLD.TO_DATE <> NEW.TO_DATE OR OLD.BREAK <> NEW.BREAK 
BEGIN
	UPDATE TIME_SHEET
	SET USED_TIME = COALESCE(NEW.TO_DATE,OLD.TO_DATE,0) - COALESCE(NEW.FROM_DATE,OLD.FROM_DATE,0) - COALESCE(NEW.BREAK,OLD.BREAK,0) - 900
