...
DROP TRIGGER IF EXISTS CHECK_PERSON2;
DROP TRIGGER IF EXISTS CHECK_PERSON3;
DROP TRIGGER IF EXISTS CHECK_PERSON4;
...
BEGIN
    SELECT RAISE(ABORT, 'You can''t add person with empty IS_FROM_POOL.');
END;

CREATE TRIGGER CHECK_PERSON2 BEFORE UPDATE ON PERSON
WHEN
	NEW.IS_FROM_POOL IS NULL
BEGIN
    SELECT RAISE(ABORT, 'You can''t update person with empty IS_FROM_POOL.');
END;

CREATE TRIGGER CHECK_PERSON3 BEFORE INSERT ON PERSON
WHEN
	(NEW.ROLE_CODE LIKE '%VE%' OR NEW.ROLE_CODE LIKE '%CN%') AND NEW.RANK_CODE <> 'NONE'
BEGIN
    SELECT RAISE(ABORT, 'You can''t add person with such combination of ROLE_CODE and RANK_CODE.');
END;

CREATE TRIGGER CHECK_PERSON4 BEFORE UPDATE ON PERSON
WHEN
	((NEW.ROLE_CODE LIKE '%VE%' OR NEW.ROLE_CODE LIKE '%CN%') AND NEW.RANK_CODE <> 'NONE'
BEGIN
    SELECT RAISE(ABORT, 'You can''t update person with such combination of ROLE_CODE and RANK_CODE.');
END;