...
BEGIN
    INSERT INTO WORK_ORDER_HIST ( ID, WORK_NO, STATUS_CODE, COMMENT, DESCRIPTION, COMPLEXITY_CODE, COMPLEXITY, PRICE, LAST_MOD, HIST_CREATE )
    VALUES ( OLD.ID, OLD.WORK_NO, OLD.STATUS_CODE, OLD.COMMENT, OLD.DESCRIPTION, OLD.COMPLEXITY_CODE, OLD.COMPLEXITY, OLD.PRICE, OLD.LAST_MOD, STRFTIME('%s','NOW') );
...
DROP TRIGGER IF EXISTS SET_LAST_MOD_WORK_ORDER;
CREATE TRIGGER SET_LAST_MOD_WORK_ORDER AFTER INSERT ON WORK_ORDER
WHEN
    NEW.LAST_MOD IS NULL
BEGIN
    UPDATE WORK_ORDER SET LAST_MOD = STRFTIME('%s','NOW');
END;
