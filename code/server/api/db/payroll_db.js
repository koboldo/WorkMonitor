...
const queries = {
    ...
    , PERIOD_WO AS (
        ...
        FROM WORK_ORDER Q JOIN (
            ...
            SELECT PW.WO_ID ID, COUNT(P.ID) ALL_CNT, SUM(CASE P.IS_FROM_POOL WHEN "Y" THEN 1 ELSE 0 END) POOL_CNT 
                , CAST(SUM(CASE P.IS_FROM_POOL WHEN "Y" THEN 1 ELSE 0 END) AS DOUBLE)/CAST(COUNT(P.ID) AS DOUBLE) POOL_INCLUSION_FACTOR
            FROM PERIOD_PERSON P JOIN PERSON_WO PW ON P.ID = PW.PERSON_ID JOIN WORK_ORDER WO ON PW.WO_ID = WO.ID
            GROUP BY PW.WO_ID
        ) G ON Q.ID = G.ID
        WHERE Q.STATUS_CODE <> 'TR'
    )
    , PERIOD_STATS AS (
       SELECT CAST(COALESCE(
            (SELECT ROUND(SUM(CAST(PRICE AS DOUBLE) * PIF), 2) FROM PERIOD_WO WHERE IS_FROM_POOL = 'Y')
    ...
    , PERSON_STATS_INIT AS (
        ...
        FROM (
            SELECT
                ...
                , (SELECT GROUP_CONCAT(PWO.ID || ':' || PWO.PRICE || '::' || PWO.PIF || ':::' || PWO.IS_FROM_POOL,'|') FROM PERIOD_WO PWO JOIN PERSON_WO PW ON PWO.ID = PW.WO_ID WHERE PW.PERSON_ID = P.ID) COMPLETED_WO 
