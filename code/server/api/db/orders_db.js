...
const queries = {
    ...
    getProtocolNOs: `SELECT PROTOCOL_NO, CASE LENGTH(PROTOCOL_NO) 
                            WHEN 8 THEN SUBSTR(PROTOCOL_NO,2,2) || SUBSTR(PROTOCOL_NO,5,4) 
                            WHEN 12 THEN SUBSTR(PROTOCOL_NO,3,2) || SUBSTR(PROTOCOL_NO,6,2) || SUBSTR(PROTOCOL_NO,9,4) 
                            ELSE "0001"
                        END LEN 
    FROM WORK_ORDER WHERE PROTOCOL_NO IS NOT NULL AND PROTOCOL_NO <> "" GROUP BY PROTOCOL_NO ORDER BY 2 DESC`,
...
const filters = {
    getOrders: {
        ...
        protocolNo: 'PROTOCOL_NO = "%(protocolNo)s"'
    },
    calculateTotalPrice: {
