...
const queries = {
	...
	PERSON_INIT AS (
		SELECT ID, ROWID, IS_FROM_POOL, FIRST_NAME, LAST_NAME, EMAIL, OFFICE_CODE, ROLE_CODE, LAST_MOD
			 FROM (
			SELECT ID
		...
		FROM PERSON_HIST
		WHERE IS_DELETED = 'N'
		) WHERE ROLE_CODE LIKE '%%EN%%' OR ROLE_CODE LIKE '%%MG%%' OR ROLE_CODE LIKE '%%OP%%' OR ROLE_CODE LIKE '%%CN%%'
	),
	PERIOD_PERSON AS (
		SELECT PI.* FROM PERSON_INIT PI
		JOIN (
			SELECT ID, MAX(ROWID) MAX_ROWID
			FROM PERSON_INIT, PARAMS
			WHERE LAST_MOD < PARAMS.BEFORE_DATE
			GROUP BY ID
