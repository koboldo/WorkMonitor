...
var queries = {
	...
	SELECT WOS.ID AS WO_ID ,WOS.WORK_NO AS WO_WORK_NO ,WOS.DONE_DATE AS WO_DONE_DATE ,WOS.ASSIGNED_DATE AS WO_ASSIGNED_DATE ,WOS.STATUS_CODE AS WO_STATUS_CODE ,WOS.TYPE_CODE AS WO_TYPE_CODE, WOS.COMPLEXITY AS WO_COMPLEXITY ,WOS.COMPLEXITY_CODE AS WO_COMPLEXITY_CODE ,P.ID AS PERSON_ID ,P.EMAIL AS PERSON_EMAIL ,P.FIRST_NAME AS PERSON_FIRST_NAME ,P.LAST_NAME AS PERSON_LAST_NAME ,P.OFFICE_CODE AS PERSON_OFFICE_CODE ,P.ROLE_CODE AS PERSON_ROLE_CODE ,P.IS_FROM_POOL AS PERSON_IS_FROM_POOL
	FROM (
	SELECT ID ,WORK_NO ,CASE STATUS_CODE WHEN 'CO' THEN DATETIME ( LAST_MOD ,'unixepoch' ) ELSE NULL END AS DONE_DATE ,STATUS_CODE ,TYPE_CODE ,COMPLEXITY, COMPLEXITY_CODE ,DATETIME ( PWO.CREATED ,'unixepoch' ) AS ASSIGNED_DATE ,PWO.PERSON_ID FROM WORK_ORDER AS WO JOIN PERSON_WO AS PWO ON WO.ID = PWO.WO_ID WHERE ( STATUS_CODE = 'CO' AND LAST_MOD / 86400 BETWEEN ( SELECT AFTER_DATE / 86400 FROM PARAMS ) AND ( SELECT BEFORE_DATE / 86400 FROM PARAMS ) ) OR ( WO.STATUS_CODE IN ('AS') AND PWO.CREATED / 86400 BETWEEN ( SELECT AFTER_DATE / 86400 FROM PARAMS ) AND ( SELECT BEFORE_DATE / 86400 FROM PARAMS ) )
	UNION ALL
	SELECT ID ,WORK_NO ,CASE STATUS_CODE WHEN 'CO' THEN DATETIME ( LAST_MOD ,'unixepoch' ) ELSE NULL END AS DONE_DATE ,STATUS_CODE ,TYPE_CODE ,COMPLEXITY, COMPLEXITY_CODE ,DATETIME ( PWO.CREATED ,'unixepoch' ) AS ASSIGNED_DATE ,PWO.PERSON_ID FROM WORK_ORDER_HIST AS WO JOIN PERSON_WO AS PWO ON WO.ID = PWO.WO_ID WHERE ( STATUS_CODE = 'CO' AND LAST_MOD / 86400 BETWEEN ( SELECT AFTER_DATE / 86400 FROM PARAMS ) AND ( SELECT BEFORE_DATE / 86400 FROM PARAMS ) )
	) AS WOS
	JOIN PERSON AS P WHERE WOS.PERSON_ID = P.ID`,
	getIsFromPool: 'SELECT CASE WHEN P.IS_FROM_POOL == "Y" AND WT.IS_FROM_POOL == "Y" THEN "Y" ELSE "N" END IS_FROM_POOL FROM PERSON P, WORK_TYPE WT, WORK_ORDER WO WHERE WO.ID = ? AND P.ID = ? AND WT.OFFICE_CODE = WO.OFFICE_CODE AND WT.TYPE_CODE = WO.TYPE_CODE AND WT.COMPLEXITY_CODE = WO.COMPLEXITY_CODE'
};
